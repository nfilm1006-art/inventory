...
function aturVisibilitasHalaman() {
  ...
  if (isLoggedIn) {
    ...
    renderAplikasi(); // Render data gudang jika sukses masuk
  } else {
...
// Event handler untuk Form Login submit
...
const placeholderImg =
  "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100' viewBox='0 0 24 24' fill='none' stroke='%2364748b' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><rect width='18' height='18' x='3' y='3' rx='2' ry='2'/><circle cx='9' cy='9' r='2'/><path d='m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21'/></svg>";
...
let products =
  JSON.parse(localStorage.getItem('gudang_data_foto')) || initialInventory;
let logs = JSON.parse(localStorage.getItem('gudang_logs')) || [
  {
    teks: 'Sistem diinisialisasi berhasil.',
...
function renderAplikasi() {
  if (!isLoggedIn) return; // Jangan render apapun jika belum login
  ...
  products.forEach((item) => {
    ...
    tr.className =
      'hover:bg-slate-750 transition-colors border-b border-slate-700/50';
    tr.innerHTML = `
      <td class="px-6 py-3 whitespace-nowrap">
        <img src="${item.foto || placeholderImg}" alt="${
      item.nama
    }" class="w-12 h-12 object-cover rounded-lg bg-slate-900 border border-slate-700 shadow-inner" />
      </td>
      <td class="px-6 py-3 font-medium text-white">${item.nama}</td>
      <td class="px-6 py-3 text-center"><span class="font-mono bg-slate-900 px-2 py-0.5 rounded text-slate-400 text-xs">${
        item.rak
      }</span></td>
      ...
      <td class="px-6 py-3 text-center text-slate-400">${
        item.minimalStok
      } Pcs</td>
      <td class="px-6 py-3 text-center">
        <div class="flex justify-center gap-1">
          <button data-id="${
            item.id
          }" data-aksi="kurang" class="bg-slate-700 hover:bg-slate-600 text-white text-xs px-2.5 py-1 rounded transition font-bold">-1</button>
          <button data-id="${
            item.id
          }" data-aksi="tambah" class="bg-slate-700 hover:bg-slate-600 text-white text-xs px-2.5 py-1 rounded transition font-bold">+1</button>
        </div>
      </td>
      <td class="px-6 py-3 text-center">
        <button data-id="${
          item.id
        }" data-aksi="hapus" class="bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white text-xs px-3 py-1.5 rounded transition border border-red-500/20 flex items-center gap-1 mx-auto">🗑️ Hapus</button>
      </td>
    `;
    ...
    if (isKritis) {
      ...
      alertDiv.className =
        'bg-red-950/40 border-l-4 border-red-500 p-3 rounded-r-lg shadow-md border border-y-slate-700 border-r-slate-700 flex gap-3 items-center';
      alertDiv.innerHTML = `
        <img src="${
          item.foto || placeholderImg
        }" class="w-10 h-10 object-cover rounded bg-slate-900 border border-red-500/20" />
        <div>
          <p class="text-xs text-red-200 font-medium">⚠️ Stok ${
            item.nama
          } kritis! Sisa ${item.stok} Pcs</p>
  ...
  logs.forEach((log) => {
    ...
    logDiv.className =
      'flex justify-between items-start py-1 border-b border-slate-700/30 text-slate-300 hover:text-white';
    logDiv.innerHTML = `
...
document.getElementById('formBarang').addEventListener('submit', (e) => {
  ...
  if (fileFoto) {
    ...
    reader.onload = function (event) {
      buatBarangBaru(event.target.result);
    };
  ...
  products = products.map((item) => {
    if (item.id === id) {
      ...
      if (aksi === 'kurang') {
        ...
        catatAktivitas(`Stok "${item.nama}" berkurang menjadi ${stokBaru}`);
...
document.getElementById('btnBersihkanLog').addEventListener('click', () => {
  ...
  const inputPassword = prompt(
    '🔐 Area Terbatas! Masukkan password admin untuk menghapus log:'
  );
  ...
  if (inputPassword === PASSWORD_RAHASIA) {
    if (confirm('Apakah Anda yakin ingin menghapus seluruh log?')) {
      logs = [
        {
          teks: 'Log dibersihkan oleh Admin Utama.',
          waktu: new Date().toLocaleTimeString(),
        },
      ];
...
// Pemicu inisialisasi pengecekan halaman saat web pertama kali dibuka
aturVisibilitasHalaman();
